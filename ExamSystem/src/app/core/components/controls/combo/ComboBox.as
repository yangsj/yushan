package app.core.components.controls.combo
{
	...
	public class ComboBox extends Sprite implements IDisposable
	{
		...
		{
			apps.removeEventListener(MouseEvent.MOUSE_UP, stageMouseHandler );
			container1.visible = false;
		}
		...
		protected function skinMouseHandler( event:MouseEvent ):void
		{
			...
			else if ( type == MouseEvent.MOUSE_DOWN )
			{
				...
				if ( container1.visible )
					apps.addEventListener(MouseEvent.MOUSE_UP, stageMouseHandler );
			}
		...
		{
			apps.removeEventListener(MouseEvent.CLICK, stageClickHandler );
			container1.visible = false;
		}
